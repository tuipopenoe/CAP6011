...
//    protected int color;
    ...
    public void Illuminate(Light l[], int lights,                            
                    int v[], Surface surface,float r[],float g[],float b[])
    {
        r[0] =r[1] =r[2] = 0;
        g[0] =g[1] =g[2] = 0;
        b[0] =b[1] =b[2] = 0;
    ...
    public void fill(Light lightList[],int lights,Vertex3D worldList[]
                ,int nVertices
                ,Matrix4x4 model, Matrix4x4 view
                ,TriFace triList[], int nTriangles, Raster raster)
    {
        ...
        {
            triList[i].calColor();
            Draw(raster,triList[i].getVertix()
                ,triList[i].getR(),triList[i].getG(),triList[i].getB()
                ,triList[i].getColor());
        }
    }
    public final static int Area2(int x1,int y1
                        , int x2, int y2, int x3, int y3)
    {
       	 return x1*y2+y1*x3+x2*y3-x3*y2-y3*x1-x2*y1;
    }
    public abstract void Draw(Raster rast, int v[]
